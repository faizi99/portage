def get_text_comp_features():
	...
	folder_to_iterate = absolute_path + '/pdf_documents'
	...
	for filename in os.listdir(folder_to_iterate):
		...
		for i in range(len(images)):
			...
			s3_client.upload_file(absolute_path + f"/s3_upload_folder/{document_name}_split{i}.jpg",
								  'jonathansplitting',
	...
	container_folder = absolute_path + "/split_pdf_folders"
	for folder in os.listdir(container_folder):
		...
		path_folder = absolute_path + f"/split_pdf_folders/{folder}"
		...
		for i in range(len(os.listdir(path_folder))):
			...
			with open(absolute_path + f"/split_pdf_folders/{folder}/{folder}_split{i}.jpg", 'rb') as image:
	...
	# making a dict of these values to append in df
		dict = {'Doc': doc, 'Multi-?': multi_question_mark, 'Multi-$': multi_dollar_sign, 'Tvalues': test_counter,
				'ICD10': icd_counter, 'Meds': medication_counter, 'Multi-date': date_counter}
